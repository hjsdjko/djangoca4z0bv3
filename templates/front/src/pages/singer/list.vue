...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>歌手姓名：</div>
        <el-input v-model="formSearch.singer" placeholder="歌手姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>风格：</div>
        <el-input v-model="formSearch.fengge" placeholder="风格" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>地区：</div>
	  ...
	  <el-button v-if="btnAuth('singer','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 10px","margin":"0px 10px 0 0","color":"#fff","borderRadius":"0px","background":"rgba(30,124,9,1)","display":"inline-block","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/singerAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","textAlign":"center","flex":"1"}'>
				...
				<div class="title">{{item.biaoqian}}</div>
				<div class="title">{{item.fengge}}</div>
		      ...
		      <div class="desc" v-html="item.gerenzhuanji"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/biaoqian/biaoqian').then(res => {
			if (res.data.code == 0) {
				this.biaoqianOptions = res.data.data;
				this.selectOptionsList.push({name:'标签',list:this.biaoqianOptions,tableName: 'biaoqian',check: -1})
			}
		});
		this.$http.get('option/zimu/zimu').then(res => {
			if (res.data.code == 0) {
				this.zimuOptions = res.data.data;
				this.selectOptionsList.push({name:'字母',list:this.zimuOptions,tableName: 'zimu',check: -1})
			}
		});
	    this.geshouxingbieOptions = '男,女'.split(',');
		this.selectOptionsList.push({name:'歌手性别',list:this.geshouxingbieOptions,tableName: 'geshouxingbie',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getFenlei() {
		this.$http.get('biaoqian/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'biaoqian'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.zimu != '') searchWhere.zimu = this.formSearch.zimu;
        if (this.formSearch.fengge != '') searchWhere.fengge = '%' + this.formSearch.fengge + '%';
        ...
        if (this.curFenlei != '全部') searchWhere.biaoqian = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`singer/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/singerDetail', query: params});
      },
